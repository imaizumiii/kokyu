...
\end{equation}
とする．
ユニタリ演算子の固有値は絶対値が$1$である．
そのため，固有値は円周上の角度$\phi$で表せる．この角度を固有位相という．
\textbf{量子位相推定}は，$U$と$\ket{u}$を入力とし，
$\phi$の$t$ビット近似$\widetilde{\phi}$を別の量子ビットへ記録する処理である
\cite{shimada2020}．
...
\end{align*}
となる．
指数$2^r$を変えると，$\phi$の2進表示の異なる桁を調べることができる．
...
\end{align}
となる．
これにより，固有位相$\phi$の情報が位相レジスタ全体に記録される．
...
\end{equation}
と定義する．
位相差を$\delta_y\coloneq\phi-y/M$とおくと，有限等比級数より
\begin{equation}
  |\alpha_y|^2
  =
  \frac{1}{M^2}
...
実際の量子アルゴリズムでは，入力が一つの固有状態であるとは限らない．
演算子$U$の正規直交固有状態を$\ket{u_j}$とし，一般の入力状態を
\begin{equation}
  \ket{\psi}
  \coloneq
  \sum_j c_j\ket{u_j}
...
状態レジスタには，測定結果に対応する$\ket{u_j}$の成分が残る．
一方，測定を行わずに位相レジスタを制御情報として用いれば，
すべての固有状態の成分を，重ね合わせを保ったまま処理できる．
...
一方，線形方程式の行列$A$は，一般にはユニタリではない．
行列$A$がエルミートであれば，実数$t_0$に対して
\begin{equation}
  U_A
  \coloneq
  e^{\mathrm{i}At_0}
...
位相の情報を使った処理がすでに終わっていれば，その処理の結果は残る．
一方，位相レジスタだけは初期状態に戻る．
HHLアルゴリズムでは，固有値に応じた制御回転の後に逆量子位相推定を行い，
位相レジスタを初期状態へ戻して解状態から切り離す．
