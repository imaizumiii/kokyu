...
離散フーリエ変換を量子状態に対して行う演算を
\textbf{量子フーリエ変換}という\cite{shimada2020}．
量子ビット数$t$を正の整数とし，計算基底状態の総数を
\begin{equation}
  M
  \coloneq
  2^t
...
\end{align}
と変形できる．第1式から第2式では，有限和の順序を入れ替えた．
さらに，入力振幅$x_j$ごとに項をまとめた．一方，入力状態は
...
\end{equation}
となる．次に，第2量子ビットを制御側，第1量子ビットを操作される側とする
制御$R_2$ゲートを作用させると，$j_2=1$の場合に位相$2\pi/2^2$が加わる．
さらに，第3量子ビットから第$t$量子ビットまでを制御側とする
...
\begin{figure}[tbp]
  ...
  \caption{4量子ビット量子フーリエ変換の回路．ゲート$R_\ell$は式\eqref{eq:qft-phase-gate}で
  定義した位相ゲートであり，黒丸と$R_\ell$を結ぶ線は制御$R_\ell$ゲートを表す．
  右端の$\times$印はSWAPゲートを表す．}
  \label{fig:qft-circuit-four-qubit}
\end{figure}

量子フーリエ変換の逆変換は，共役転置$F_M^\dagger$であり，
計算基底に対して
\begin{equation}
  F_M^\dagger\ket{k}
  =
...
近似を用いない場合を考える．$t$量子ビットの量子フーリエ変換では，
第$r$量子ビットに1個のHadamardゲートと$t-r$個の制御位相ゲートを用いる．
したがって，
SWAPゲートを除くゲート数は
\begin{equation}
  \sum_{r=1}^{t}
...
$3\lfloor t/2\rfloor$個のCNOTゲートを用いる．したがって，この量子フーリエ変換の
ゲート数は
\begin{equation}
  O(t^2)
  =
...
一方をよくすると他方が悪くなるため，後の処理で必要となる位相の精度に応じて
どこまで省略するかを決める．
