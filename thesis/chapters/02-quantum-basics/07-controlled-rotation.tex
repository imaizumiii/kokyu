...
数値$z$を保存する量子ビットのまとまりを値レジスタという．
値レジスタの計算基底を$\{\ket z\}$とし，
実数値関数$g$がすべての$z$について$|g(z)|\leq1$を満たすとする．
値$z$に応じて補助量子ビットの状態を変える演算$W_g$を
\begin{equation}
 W_g\ket z\ket0
 =\ket z\left(
...
\end{prop}
\begin{proof}
式\eqref{eq:controlled-rotation-block}を見ると，値$z$ごとに
補助量子ビットへユニタリ演算$R_y(2\arcsin g(z))$を加えている．
異なる$z$に対応する部分空間は互いに直交する．
したがって，$W_g$はブロック対角な演算である．実際，
\begin{align*}
 W_g^\dagger W_g
...
測定結果$1$に対応する第2項を$\sqrt{p_1}$で割り，長さを1にすると，
