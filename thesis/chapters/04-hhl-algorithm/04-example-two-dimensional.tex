...
\end{equation}
とする．状態$\ket\pm=(\ket0\pm\ket1)/\sqrt2$を用いる．
直接計算すると，
\[
 A\ket+=\ket+,
...
ユニタリ演算$U=e^{\mathrm{i}A\pi}$を位相推定に用いる．式\eqref{eq:matrix-eigenvalue-to-phase}より，
固有値$1$の位相は$1/2=0.10_2$，固有値$1/2$の位相は$1/4=0.01_2$である．
よって2量子ビットの位相レジスタで両方を正確に表せる．

定数を$C=1/2$と選ぶ．補助量子ビットの$\ket1$に掛かる係数
$C/\lambda$は，
...
\]
であるから，これが成功確率である．成功時に得られる長さ1の状態は
\begin{align}
 \frac{1}{\sqrt{5/8}}
 \frac1{\sqrt2}\left(\frac12\ket++\ket-\right)
...
\]
である．これを長さ1にそろえた状態は式\eqref{eq:hhl-example-output}と一致する．
\end{ex}
