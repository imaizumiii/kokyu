...
量子系が測定を受けずに閉じている間，その状態変化はユニタリ写像で表されるとする．
...
\[
  ...
  =\braket{\phi|\psi}
\]
である．$\ket{\phi}=\ket{\psi}$とすればノルムの保存が従う．
また$U^\dagger U=UU^\dagger=I$は，$U^\dagger$が$U$の両側逆写像であることを表す．
\end{proof}
...
\end{align}
が成り立つ．一般のテンソルは純テンソル$v\otimes w$の有限和で表されるが，
必ずしも一つの純テンソルにはならない．

\begin{prop}
$\{e_i\}_{i=1}^{r}$を$V$の基底，$\{f_j\}_{j=1}^{s}$を$W$の基底とすると，
$\{e_i\otimes f_j\}_{i,j}$は$V\otimes W$の基底である．
したがって$\dim(V\otimes W)=rs$である．
\end{prop}
\begin{proof}
任意の$v=\sum_i a_ie_i$，$w=\sum_jb_jf_j$について，双線形性より
\[
  v\otimes w=\sum_{i,j}a_ib_j(e_i\otimes f_j)
\]
である．一般のテンソルは純テンソルの有限和であるから，$e_i\otimes f_j$は
$V\otimes W$を生成する．

次に座標双線形写像$f_{kl}:V\times W\to\mathbb C$を
$f_{kl}(v,w)=a_kb_l$で定める．普遍性より，
...
$\widetilde f_{kl}$を作用させて$c_{kl}=0$を得る．$k,l$は任意であるから線形独立である．
よってこれは$rs$個の基底ベクトルからなる．
\end{proof}
...
複合量子系の状態空間は，各部分系の状態空間のテンソル積で与えられるとする．
たとえば2量子ビットの計算基底は
\[
...
\begin{defi}[制御ユニタリ]
1量子ビットを制御系，状態空間$\mathcal K$を標的系とするとき，
\begin{equation}
...
\begin{prop}
$U$がユニタリなら$C(U)$もユニタリである．
\end{prop}
\begin{proof}
$P_0=\ket{0}\bra{0}$，$P_1=\ket{1}\bra{1}$と置くと，
...
$U=X$の場合がCNOTゲートである．また，制御量子ビットが
$\alpha\ket{0}+\beta\ket{1}$，標的が$\ket{\psi}$なら，線形性より
\begin{equation}
